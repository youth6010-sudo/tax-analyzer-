...
export function CalendarDayCell({
  ...
  return (
    <button
      ...
      className={`border-b border-r border-slate-100 p-1 text-left transition-colors ${
        tall ? 'min-h-[11rem]' : 'min-h-[8rem]'
      } ${inMonth ? 'bg-white' : 'bg-slate-50/80'} ${
        isSelected ? 'ring-2 ring-inset ring-blue-400' : 'hover:bg-blue-50/40'
      }`}
    >
      <span
