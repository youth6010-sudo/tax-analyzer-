// 블루홀 거래처 단건 조회/수정 (허브)
//   GET   → { client: { id, name, values, labels, ... } }  (35필드 전체)
//   PATCH { changes: { <컬럼>: 값 } } → updateInfo (CLIENT_FIELDS 화이트리스트)
...
const EDITABLE_COLUMNS: ReadonlySet<string> = new Set(
  (bh.CLIENT_FIELDS as ClientField[]).map((f) => f.col),
);
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const client = await withBluehole(a.user.id, (cookie) => bh.getClient(cookie, id));
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = (await withBluehole(a.user.id, (cookie) => bh.updateClient(cookie, id, changes))) as {
      success_cols?: string[];
      warnings?: string[];
    };
