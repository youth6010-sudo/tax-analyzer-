...
export function applyConsultationLinks(form: Record<string, string>): Record<string, string> {
  ...
  const meeting = form.consultDate?.trim() || form.meetingDate?.trim();
  if (meeting) next.recordMeetingAt = meeting;
  if (form.consultDate?.trim() && !form.meetingDate?.trim()) {
    next.meetingDate = form.consultDate.trim();
  }
  ...
  if (form.consultDate?.trim()) {
    const when = [form.consultDate, form.consultAmPm, form.consultContactType].filter(Boolean).join(' ');
    phoneLines.push(`상담일시: ${when}`);
  }
  push('유입', form.channel);
  if (form.channelDetail?.trim()) phoneLines.push(form.channelDetail.trim());
  if (form.requestDetails?.trim()) phoneLines.push(`요청: ${form.requestDetails.trim()}`);
  ...
  push('대면 필요', form.needMeeting);
  push('대면 예정', form.meetingDate);
  push('장소', form.meetingPlace);
  if (form.docsToBring?.trim()) phoneLines.push(`준비 자료: ${form.docsToBring.trim()}`);
